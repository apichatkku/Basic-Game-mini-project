...
Level.prototype.preload = function() {
	this.stage.backgroundColor = "#A9BCF5";
	...
	this.ahh.allowMultiple=false;
	this.eatAudio1 = this.add.audio("eatAudio1",0.5,false);
	this.eatAudio1.allowMultiple=false;
...
//get parametor
Level.prototype.init = function(pPlayer){
...
Level.prototype.create = function() {
	...
	/*this.gates.forEachAlive(function(g){
		if(g.name=="go"&&this.comefromLevel>this.mapNumber){
			this.player.x = g.x;
			this.player.y = g.y;
		}
	}, this);*/
	...
	this.time.events.loop(500,function(){
		for(var i = 0 ; i < 10 ; i++){
			var a = this.add.sprite(Math.random()*this.world.width,0,"rain",0,this.Rain);
			a.anchor.set(0.5,1);
			...
			a.animations.add("boom",[1,2],12,true);
		}
...
Level.prototype.update = function() {
	...
	this.physics.arcade.collide(this.Rain,this.map_border,function(rain){
		rain.play("boom",12,false,true);
	},null,this);
	if(this.player.isDead) return;
	
	this.enemies.forEachAlive(function(enemy){
		if(enemy.isHit||enemy.isAttack||enemy.isDead) return;
		if(this.math.distance(this.player.x,this.player.y,enemy.x,enemy.y)<400){
			if(this.player.x>enemy.x){
				enemy.body.velocity.x = enemy.speed;
			}else{
				enemy.body.velocity.x = -enemy.speed;
			}
			if(this.player.y+200<enemy.y&&enemy.body.onFloor()){
				...
				this.enemies.forEachAlive(
						function(enemy){
							...
							if(Phaser.Rectangle.intersects(rec1,enemy.getBounds())){
								...
								this.addDamageText(tmpDamage,enemy.x,enemy.y-enemy.height,"Orange");
	...
	this.physics.arcade.overlap(this.actors,this.itemGroup,this.pickItem,null,this);
...
Level.prototype.pickItem = function(player,item){
...
Level.prototype.playerCollideEnemies = function(player,enemy){
	if(enemy.isDead) return;
	player.canHit = false;
	enemy.myAttack();
	this.ahh.play();
	...
	this.addDamageText(tmpDamage,player.x,player.y-player.height,"Red");
...
Level.prototype.addEffBlood = function(x,y,w){
...
Level.prototype.addDamageText = function(damage,x,y,tcolor){
	...
	var dy = y-100;
	var newH = text.height*1.5;
	var newW = text.width*1.5;
    var twText = this.add.tween(text);
    twText.to({ y:dy , width:newW , height:newH  }, 300, "Linear", true);
...
Level.prototype.addHealText = function(damage,x,y,tcolor){
...
Level.prototype.goMap = function(player,gate){
	if(gate.name=="go"){
